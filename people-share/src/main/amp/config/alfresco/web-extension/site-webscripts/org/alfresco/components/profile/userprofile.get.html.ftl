...
<script type="text/javascript">//<![CDATA[
   var userProfile = new SomeCo.UserProfile("${args.htmlid}").setOptions(
   {
      userId: "${user.name}",
      profile: {
         ...
         birthDate: "<#if profile.properties.birthDate??>${profile.properties.birthDate?js_string}</#if>",
         shirtSize: "<#if profile.properties.shirtSize??>${profile.properties.shirtSize?js_string}</#if>",
         significantOther: "<#if profile.properties.significantOther??>${profile.properties.significantOther?js_string}</#if>",
         likesNeilDiamond: "<#if profile.properties.likesNeilDiamond??>${profile.properties.likesNeilDiamond?js_string}</#if>"
      }
   }).setMessages(
      ${messages}
   );
      ...
      <div id="${el}-body" class="profile">
         <div id="${el}-readview" class="hidden">
            ...
            <@markup id="viewCompanyInfo">
               <div class="header-bar">TT${msg("label.companyinfo")}</div>
            ...
            <@markup id="viewSomeCoInfo">
            	<div class="header-bar">${msg("label.somecoInfo")}</div>
            	<#if profile.properties.birthDate?? && profile.properties.birthDate?length!=0>
		         <div class="row">
		            <span class="fieldlabelright">${msg("label.birthDate")}:</span>
		            <span class="fieldvalue">${profile.properties.birthDate?html}</span>
		         </div>
		         </#if>
		         <#if profile.properties.shirtSize?? && profile.properties.shirtSize?length!=0>
		         <div class="row">
		            <span class="fieldlabelright">${msg("label.shirtSize")}:</span>
		            <span class="fieldvalue">${profile.properties.shirtSize?html}</span>
		         </div>
		         </#if>
		         <#if profile.properties.significantOther?? && profile.properties.significantOther?length!=0>
		         <div class="row">
		            <span class="fieldlabelright">${msg("label.significantOther")}:</span>
		            <span class="fieldvalue">${profile.properties.significantOther?html}</span>
		         </div>
		         </#if>
		         <#if profile.properties.likesNeilDiamond?? && profile.properties.likesNeilDiamond?length!=0>
		         <div class="row">
		            <span class="fieldlabelright">${msg("label.likesNeilDiamond")}:</span>
		            <span class="fieldvalue">${profile.properties.likesNeilDiamond?html}</span>
		         </div>
         		</#if>
            </@markup>
            </div>
         ...
         <div id="${el}-editview" class="hidden">
            ...
            <div class="row">
	      	...
	      	<div class="row">
	         	<span class="label"><label for="${el}-input-shirtSize">${msg("label.shirtSize")}:</label></span>
	         	<span><select id="${el}-input-shirtSize"><option value="XS">XS</option><option value="S">S</option><option value="M">M</option><option value="L">L</option><option value="XL">XL</option></select></span>
	      	</div>
		    <div class="row">
		         <span class="label"><label for="${el}-input-significantOther">${msg("label.significantOther")}:</label></span>
		         <span><input type="text" maxlength="256" size="30" id="${el}-input-significantOther" value="" <@immutablefield field="significantOther" /> /></span>
		    </div>
		    <div class="row">
		         <span class="label"><label for="${el}-input-likesNeilDiamond">${msg("label.likesNeilDiamond")}:</label></span>
		         <span><select id="${el}-input-likesNeilDiamond"><option value="true">Yes</option><option value="false">No</option></select></span>
		    </div>
            </@markup>
            <hr/>
